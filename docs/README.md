# Artharum

Selamat datang di dokumentasi lengkap **Artharum**.

---

## Quick Start

### Untuk Pengguna Aplikasi
Ingin menggunakan aplikasi? Mulai dari sini:
- **[Installation & User Guide](wiki/Installation.md)** - Download, install, dan tutorial lengkap
- **[Features](wiki/Features.md)** - Penjelasan semua fitur
- **[FAQ](wiki/FAQ.md)** - Jawaban atas pertanyaan umum

### Untuk Developer
Ingin berkontribusi atau develop? Mulai dari sini:
- **[Development Setup](wiki/Development-Setup.md)** - Setup environment
- **[Architecture](wiki/Architecture.md)** - System design
- **[Contributing](wiki/Contributing.md)** - Cara kontribusi

### Untuk DevOps / Release Manager
Ingin deploy atau release? Mulai dari sini:
- **[GitHub Actions](wiki/GitHub-Actions.md)** - CI/CD pipeline

---

## Daftar Lengkap

| File | Deskripsi |
|------|-----------|
| **[Home.md](wiki/Home.md)** | Wiki homepage dan overview |
| **[Index.md](wiki/Index.md)** | Daftar lengkap semua dokumentasi |
| **[Installation.md](wiki/Installation.md)** | User guide & installation |
| **[Features.md](wiki/Features.md)** | Feature list & explanation |
| **[Architecture.md](wiki/Architecture.md)** | System design & patterns |
| **[Project-Structure.md](wiki/Project-Structure.md)** | Folder organization |
| **[Development-Setup.md](wiki/Development-Setup.md)** | Dev environment setup |
| **[GitHub-Actions.md](wiki/GitHub-Actions.md)** | CI/CD pipeline |
| **[Contributing.md](wiki/Contributing.md)** | Contribution guide |
| **[FAQ.md](wiki/FAQ.md)** | FAQ & troubleshooting |

---

## Cara Menggunakan Wiki

### Method 1: Baca di GitHub
1. Buka repository di GitHub
2. Klik tab **Wiki** (jika enabled)
3. Navigate menggunakan sidebar

### Method 2: Baca Local Files
1. Files tersimpan di `docs/wiki/`
2. Buka file `.md` dengan text editor atau Markdown viewer
3. Click links untuk navigate

### Method 3: Search
- **GitHub**: Gunakan search di GitHub wiki
- **Local**: Gunakan Ctrl+F dalam file

---

## Wiki Statistics

- **Total Pages**: 10+
- **Total Sections**: 50+
- **Code Examples**: 100+
- **Diagrams**: 10+
- **Last Updated**: April 29, 2026

---

## Learning Paths

### Path 1: User Onboarding
Untuk pengguna baru aplikasi:
```
Installation.md 
  → Setup Awal
  → Features Tutorial
  → Tips & Tricks
  → FAQ
```

### Path 2: Developer Onboarding
Untuk developer baru:
```
Development-Setup.md
  → Architecture.md
  → Project-Structure.md
  → Contributing.md
```

### Path 3: Quick Troubleshooting
Untuk problem solving:
```
FAQ.md
  → Troubleshooting sections
  → Solution steps
```

---

## Features

**Comprehensive** - Dokumentasi lengkap untuk semua aspek  
**Well-Organized** - Easy navigation dengan clear structure  
**Up-to-Date** - Regular updates dengan latest info  
**Examples** - Code examples dan usage tutorials  
**Searchable** - Easy to find what you need  
**Beginner-Friendly** - Clear explanations untuk semua level  

---

## Related Resources

- **GitHub Repository**: https://github.com/afadh/CatatanKeuanganPribadi
- **Issues**: https://github.com/afadh/CatatanKeuanganPribadi/issues
- **Discussions**: https://github.com/afadh/CatatanKeuanganPribadi/discussions
- **Releases**: https://github.com/afadh/CatatanKeuanganPribadi/releases

---

## Need Help?

- **Ask Questions**: [GitHub Discussions](https://github.com/afadh/CatatanKeuanganPribadi/discussions)
- **Report Issues**: [GitHub Issues](https://github.com/afadh/CatatanKeuanganPribadi/issues)
- **Email Support**: support@catatankeuangan.com

---

## Contributing to Wiki

Ingin improve wiki?
- Fix typos atau grammar
- Improve clarity
- Add new sections
- Submit via Pull Request

Lihat [Contributing.md](wiki/Contributing.md) untuk details.

---

**Last Updated**: April 29, 2026  
**Version**: 1.0.0  
**Status**: Production Ready

---

**Happy Reading!**
